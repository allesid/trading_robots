...
-- is_run = false	
	...
	while is_run do
		...
		for row, SEC in pairs(sec_list) do
			...
			if tonumber(qt.bid_count)>=qstep_min and tonumber(qt.offer_count)>=qstep_min  then
				qsummax = math.min(qt.bid_count, qt.offer_count)
...
-- ===============================================
				...
				for i=1,qsummax-qstep_min do 
					...
					if qt_bid_sum*dpart < qt_of_sum then
						if last_deal[SEC][1]>=(1-dub_up) then
							...
							prc_deal = (prc_off*price_p + tonumber(qt.offer[j].price)*(1.0-price_p))
					...
					elseif qt_bid_sum > qt_of_sum*dpart then
						if last_deal[SEC][1]<=0 then
							PRICE_STEP=getSecurityInfo(CLASS, SEC).min_price_step
							prc_deal = (prc_bid*price_p + tonumber(qt.bid[1+qt.bid_count-j].price)*(1.0-price_p))
				...
				if ss_last_deal[row] > 0 then
					-- if last_deal[SEC][6] < ss_last_deal[row] then
						ss_last_deal[row] = ((ma_par - 1) * ss_last_deal[row] + 2 * last_deal[SEC][6]) / (ma_par + 1)
...
function position_up(SEC, last_dealu)
	local last_deal = last_dealu
		...
		ld = 1
		if last_deal[SEC][5] ~= 0 and last_deal[SEC][1] < 0  then
