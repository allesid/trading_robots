...
price_part = 0.03 -- цена выставления ордера между средним (бид и оффер) и бид/оффер макс в интервале [0.0-1.0]
price_partc = 0.03 -- то же для сделки закрытия позиции
...
-- is_run = false	
	while is_run do
		...
		if os.sysdate().min ~= tm_minute then
			...
			SetCell(tb_res, row_sum, 15, string.format('%.2f',tostring(summary_res[3])))
			-- total(sec_list, last_deal, ds)
	...
	if class_code==CLASS and SEC==sec_code and (tm<stime or tm>=etime) and tm>=start_time and tm<end_time then
		...
		if tonumber(qt.bid_count)>=qsummax and tonumber(qt.offer_count)>=qsummax  then
			...
			for i=1,qsummax do 
				...
				if last_deal[i][4] >= resc then
					summary_res[6] = i
					summary_res[1] = last_deal[i][1]
					resc = last_deal[i][4]
				end
			...
			if summary_res[1] > summary_res[7] then				if summary_res[7] < 0 then
					res_1 = summary_res[2]
					...
					summary_res[5] = 0
					filer_res:write(SEC.." BUY1: date="..dt.." time="..tm.." res="..summary_res[2].." price="..tostring(prc_off).."  ir="..tostring(summary_res[6]), '\n')
				elseif summary_res[7] == 0 then
					summary_res[1] = 1
					summary_res[3] = summary_res[3]-(summary_res[1] + comis)*prc_off
					summary_res[5] = prc_off
					filer_res:write(SEC.." BUY0: date="..dt.." time="..tm.." price="..tostring(summary_res[5]).."  ir="..tostring(summary_res[6]), '\n')
				end
			elseif summary_res[1] < summary_res[7] then				if summary_res[7] > 0 then
					res_1 = summary_res[2]
					...
					summary_res[5] = 0.
					filer_res:write(SEC.." SELL1: date="..dt.." time="..tm.." res="..summary_res[2].." price="..tostring(prc_bid).."  ir="..tostring(summary_res[6]), '\n')
				elseif summary_res[7] == 0 then
					summary_res[1] = -dub_up
					summary_res[3] = summary_res[3] - (summary_res[1] + comis)*prc_bid*dub_up
					summary_res[5] = -prc_bid * dub_up
					if dub_up == 1 then
						filer_res:write(SEC.." SELL0: date="..dt.." time="..tm.." price="..tostring(prc_bid).."  ir="..tostring(summary_res[6]), '\n')
					end
				end
			end
