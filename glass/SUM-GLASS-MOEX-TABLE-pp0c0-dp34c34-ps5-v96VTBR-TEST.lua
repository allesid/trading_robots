...
dpart = 34 --коэффициент превышения суммы объема бид и оффер для сделки открытия позиции
-- например: сумма бидов умноженная на dpart > суммы офферов - покупка
dpartc = 34 -- то же для сделки закрытия позиции
		...
		if tonumber(qt.bid_count)>=qsummax and tonumber(qt.offer_count)>=qsummax  then
			...
			for i=1,qsummax do 
				...
				if last_deal[i][4] >= resc then
					...
					resc = last_deal[i][4]
					-- message(SEC.." resc="..resc.." ir="..i.." ld="..summary_res[1], 1)
				end
