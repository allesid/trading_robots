...
dpart = 2 --коэффициент превышения суммы объема бид и оффер для сделки открытия позиции
-- например: сумма бидов умноженная на dpart > суммы офферов - покупка
dpartc = 2 -- то же для сделки закрытия позиции
ver = "v8-TEST" -- "WORK"
...
-- last_deal[][SEC][7] - last order price [-0]
	...
	sec_list = sl
	for tab_num, SEC in pairs(sec_list) do
		-- message("Table SEC="..SEC.." i="..i, 1)
		tb_res[tab_num] = AllocTable()
		ac_res = AddColumn(tb_res[tab_num], 1, "SEC", true, QTABLE_CACHED_STRING_TYPE , 8)
		ac_res = AddColumn(tb_res[tab_num], 2, "CLASS", true, QTABLE_CACHED_STRING_TYPE , 10)
		ac_res = AddColumn(tb_res[tab_num], 3, "Name", true, QTABLE_CACHED_STRING_TYPE, 17)
		ac_res = AddColumn(tb_res[tab_num], 4, "spred", true, QTABLE_CACHED_STRING_TYPE, 10)
		...
		for i=1,qsummax do 
			ac_res = AddColumn(tb_res[tab_num], 2*i+3, tostring(i).." RES %", true, QTABLE_CACHED_STRING_TYPE, 10)
			ac_res = AddColumn(tb_res[tab_num], 2*i+4, tostring(i).." DEAL", true, QTABLE_CACHED_STRING_TYPE, 6) 
		end
		CreateWindow(tb_res[tab_num])
			SetWindowCaption(tb_res[tab_num], "RESULT mult".." "..EXCH.."-pp"..tostring(price_part).."c"..tostring(price_partc).."-dp"..tostring(dpart).."c"..tostring(dpartc).." "..ver)
	
		row = InsertRow( tb_res[tab_num], -1)
		SetCell(tb_res[tab_num], row, 1, SEC)
		SetCell(tb_res[tab_num], row, 2, CLASS)
		SetCell(tb_res[tab_num], row, 3, getSecurityInfo(CLASS, SEC).short_name )
		last_row = InsertRow( tb_res[tab_num], -1)
		SetCell(tb_res[tab_num], last_row, 3, "Aver res summs back, %")
	end
		...
		if os.sysdate().min ~= tm_minute then
			...
			for tab_num, SEC in pairs(sec_list) do
				...
				if tonumber(qt.bid_count)>=qsummax and tonumber(qt.offer_count)>=qsummax  then
					...
					SetCell(tb_res[tab_num], row, 4, string.format('%.2f',(prc_off/prc_bid-1)*100).."%")
					if prc_bid < prc_off and  prc_off < prc_bid*1.001 then  
						SetColor(tb_res[tab_num], row, 4, RGB(255,255,255),RGB(0,0,0), RGB(255,255,255), RGB(0,0,0))
					else
						SetColor(tb_res[tab_num], row, 4, RGB(255,160,160),RGB(0,0,0), RGB(255,160,160), RGB(0,0,0))
					end
				else
					SetCell(tb_res[tab_num], row, 4, "no quote")
					-- SetColor(tb_res[tab_num], row, 8, RGB(160,160,160),RGB(0,0,0), RGB(160,160,160), RGB(0,0,0))
				end
				for i=1, qsummax do 
					if last_deal[i][SEC][1] ~= 0 then
						SetCell(tb_res[tab_num], row, 2*i+4, tostring(last_deal[i][SEC][1]))
					else
						if last_deal[i][SEC][7] ~= 0 then
							SetCell(tb_res[tab_num], row, 2*i+4, "or"..tostring(last_deal[i][SEC][7]))
						end
						if last_deal[i][SEC][6] ~= 0 then
							SetCell(tb_res[tab_num], row, 2*i+4, "or+"..tostring(last_deal[i][SEC][6]))
						end
					end
					set_sell_neg(tb_res[tab_num], row, SEC, last_deal[i], i)
				end
				qssumb20 = 0
				for i=1, qsummax do 
					j = qsummax + 1 - i
					qssumb20 = qssumb20 + last_deal[j][SEC][4]
					SetCell(tb_res[tab_num], last_row1, 2*j+3,  string.format('%.2f', qssumb20/i))
					if qssumb20 > 0 then
						SetColor(tb_res[tab_num], last_row1, 2*j+3, RGB(160,255,160),RGB(0,0,0), RGB(160,255,160), RGB(0,0,0))
					elseif qssumb20 < 0 then
						SetColor(tb_res[tab_num], last_row1, 2*j+3, RGB(255,160,160),RGB(0,0,0), RGB(255,160,160), RGB(0,0,0))
					else
						SetColor(tb_res[tab_num], last_row1, 2*j+3, RGB(255,255,255), RGB(0,0,0), RGB(255,255,255), RGB(0,0,0))
					end
					prints(sec_list, last_deal[i])
				end
			end
...
function OnQuote(class_code, sec_code)
	if class_code == CLASS then
		row = 0
		-- message("sec_code="..sec_code, 1)
		for ic, SECc in pairs(sec_list) do
			if SECc == sec_code then
				row = ic
				SEC = sec_code
				break
			end
					...
					if last_deal[i][SEC][1] < 1 then
						if last_deal[i][SEC][6] > 0 then
							if prc_off <= last_deal[i][SEC][6] then
								...
								offS = math.abs(last_deal[i][SEC][6])
								if last_deal[i][SEC][1] < 0  then
									res = last_deal[i][SEC][3]+(last_deal[i][SEC][1])*offS-math.abs(last_deal[i][SEC][1])*comis*offS
									last_deal[i][SEC][4] = last_deal[i][SEC][4] + (res - last_deal[i][SEC][2])*100/math.abs(last_deal[i][SEC][5])
									last_deal[i][SEC][2] = res
									ld = 0
									last_deal[i][SEC][5] = 0
								elseif last_deal[i][SEC][1] == 0 then
									last_deal[i][SEC][5] = last_deal[i][SEC][6]
									ld = 1
								end
								last_deal[i][SEC][3]=last_deal[i][SEC][3]-(ld-last_deal[i][SEC][1])*offS-math.abs(ld-last_deal[i][SEC][1])*comis*offS
								last_deal[i][SEC][1] = ld
--  ============================================
					...
					if last_deal[i][SEC][1] > -dub_up then
						if last_deal[i][SEC][7] < 0 then
							if prc_bid >= -last_deal[i][SEC][7] then
								...
								bidS = math.abs(last_deal[i][SEC][7])
								if last_deal[i][SEC][1] > 0 then
									res = last_deal[i][SEC][3]+(last_deal[i][SEC][1])*bidS-math.abs(last_deal[i][SEC][1])*comis*bidS
								...
								elseif last_deal[i][SEC][1] == 0 then
										ld = -dub_up
										last_deal[i][SEC][5] = last_deal[i][SEC][7] * dub_up
								end
								last_deal[i][SEC][3]=last_deal[i][SEC][3]-(ld-last_deal[i][SEC][1])*bidS-math.abs(ld-last_deal[i][SEC][1])*comis*bidS
								last_deal[i][SEC][1] = ld
--  ============================================
...
function position_up(SEC, last_deald)
	...
	last_deald[SEC][1] = ld
	if math.abs(last_deald[SEC][3]) > 1.9*offS then
		eon = EXCH.."-pp"..tostring(price_part).."c"..tostring(price_partc).."-dp"..tostring(dpart).."c"..tostring(dpartc).."-"..ver..".txt"
		efilename = path_name.."error-GLASS-"..eon
		filer_err=io.open(efilename, "a")
		filer_err:write("=== position_down2 math.abs(last_deald[SEC][3]) > 1.9*offS ==="..eon, '\n')
		filer_err:write(SEC, '\n')
		filer_err:write("res="..res, '\n')
		filer_err:write("offS="..offS)
		filer_err:write(" ld_"..last_deald[SEC][1])
...
function position_dn(SEC, last_deald)
	...
	last_deald[SEC][1] = ld
	if math.abs(last_deald[SEC][3]) > 1.9*bidS then
		eon = EXCH.."-pp"..tostring(price_part).."c"..tostring(price_partc).."-dp"..tostring(dpart).."c"..tostring(dpartc).."-"..ver..".txt"
		efilename = path_name.."error-GLASS-"..eon
		filer_err=io.open(efilename, "a")
		filer_err:write("=== position_down2 math.abs(last_deald[SEC][3]) > bidS ==="..eon, '\n')
		filer_err:write(SEC, '\n')
		filer_err:write("res="..res, '\n')
		filer_err:write("bidS="..bidS)
		filer_err:write(" ld_"..last_deald[SEC][1])
	...
	if qt and qt.offer and tonumber(qt.offer_count) ~= 0 then
		...
		if last_deald[SEC][1] ~= 0 then
			if math.abs(res - last_deald[SEC][2]) > math.abs(last_deald[SEC][5]) then
				...
				filer_err:write(SEC, '\n')
				filer_err:write("res="..res, '\n')
				filer_err:write("bidS="..bidS.." offS="..offS, '\n')
				filer_err:write(" ld_"..last_deald[SEC][1])
				filer_err:write(" res_"..last_deald[SEC][2])
				filer_err:write(" pos_"..last_deald[SEC][3])
				filer_err:write(" pospct_"..last_deald[SEC][4])
...
function stop_main(tb_res, ds)
	for i=1, qsummax do 
		...
		filer[i]:write("Stop: summary result = "..string.format('%.2f', last_deal[i][SEC][4]).." %", '\n')
		filer[i]:close()
		end_of_name = EXCH.."-M-qs"..tostring(i).."-pp"..tostring(price_part).."c"..tostring(price_partc).."-dp"..tostring(dpart).."c"..tostring(dpartc).."-"..ver..".txt"
		data_file_name = path_name.."data-GLASS-"..end_of_name
		prints(sec_list, last_deal[i])
	...
	for i=1, #sec_list do
		if not IsWindowClosed(tb_res[i]) then
			DestroyTable(tb_res[i])
		end
	end
	for i, SEC in pairs(sec_list) do
