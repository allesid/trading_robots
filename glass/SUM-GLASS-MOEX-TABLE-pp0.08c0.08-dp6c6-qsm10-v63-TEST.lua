				...
				for i=1,qstep_min-1 do 
					...
					if ss_last_deal[row] > 0 then
						if prc_off <= ss_last_deal[row] then
							...
							if ld_1 < 0  then
								res = last_deal[SEC][3] + (ld_1 - comis) * bidS
								if math.abs(last_deal[SEC][5]) ~= 0 then
									last_deal[SEC][4] = last_deal[SEC][4] + (res - last_deal[SEC][2])*100/math.abs(last_deal[SEC][5])
								end
								last_deal[SEC][1] = 0
								last_deal[SEC][3] = last_deal[SEC][3] + (ld_1 - comis) * bidS
								last_deal[SEC][2] = res
								last_deal[SEC][5] = 0
							elseif ld_1 == 0 then
								last_deal[SEC][1] = 1
								last_deal[SEC][3] = last_deal[SEC][3]-(last_deal[SEC][1]+comis)*bidS
					...
					elseif ss_last_deal[row] < 0 then
						if prc_bid >= -ss_last_deal[row] then
							...
							if ld_1 > 0 then
								res = last_deal[SEC][3]+(ld_1-comis)*offS
								if math.abs(last_deal[SEC][5]) ~= 0 then
									last_deal[SEC][4] = last_deal[SEC][4] + (res - last_deal[SEC][2])*100/math.abs(last_deal[SEC][5])
								end
								last_deal[SEC][1] = 0
							...
							elseif ld_1 == 0 then
								...
								last_deal[SEC][3] = last_deal[SEC][3]-(last_deal[SEC][1]+comis)*offS*dub_up
								last_deal[SEC][5] = last_deal[SEC][6] * dub_up
							end
...
--  ============================================
function set_sell_neg(tb_res, row, SEC, last_deald)
	-- message(SEC.." "..vers, 1)
