...
function OnInit(s)
	...
	for i=1,qsummax do 
		...
		if filer1 == nil then
			...
			message("Inserting sequrity to data file: "..data_file_name,2)
			filer1:write(SEC,'\n')
			filer1:close()
			sleep(500)
			filer1=io.open(data_file_name, "r")
				...
				if last_deal[i][1] < 1 then
					if last_deal[i][6] > 0 then
						if prc_off <= last_deal[i][6] then
							...
							if last_deal[i][1] < 0  then
								res = last_deal[i][3]+last_deal[i][1]*bidS-comis*bidS
								...
								last_deal[i][1] = 0
								last_deal[i][5] = 0
							elseif last_deal[i][1] == 0 then
								last_deal[i][3] = last_deal[i][3]-bidS-comis*bidS
								last_deal[i][5] = last_deal[i][6]
								last_deal[i][1] = 1
							end
							filer[i]:write(SEC.." BUY: date="..dt.." time="..tm.." res="..last_deal[i][2].." price="..prc_off, '\n')
						end
						last_deal[i][6] = 0. 
					end
				end
				if last_deal[i][1] > -dub_up then
					if last_deal[i][7] < 0 then
						if prc_bid >= -last_deal[i][7] then
							...
							if last_deal[i][1] > 0 then
								res = last_deal[i][3]+(last_deal[i][1])*offS-comis*offS
								if math.abs(last_deal[i][5]) ~= 0 then
									last_deal[i][4] = last_deal[i][4] + (res - last_deal[i][2])*100/math.abs(last_deal[i][5])
								end
								last_deal[i][3] = res
								...
								last_deal[i][5] = 0.
							elseif last_deal[i][1] == 0 then
								last_deal[i][3] = last_deal[i][3]+(1.-comis)*offS*dub_up
								last_deal[i][1] = -dub_up
								last_deal[i][5] = last_deal[i][7] * dub_up
							end
							filer[i]:write(SEC.." SELL: date="..dt.." time="..tm.." res="..last_deal[i][2].." price="..prc_bid, '\n')
						end
						last_deal[i][7] = 0.
					end
				end
...
function position_up(SEC, last_deald)
	...
	if last_deald[1] < 0  then
		...
		last_deald[4] = last_deald[4] + (res - last_deald[2])*100/math.abs(last_deald[5])
		last_deald[2] = res
		ld = 0
		last_deald[5] = 0
	elseif last_deald[1] == 0 then
		last_deald[5] = last_deald[6]
		ld = 1
	end
	last_deald[3]=last_deald[3]-(ld-last_deald[1])*offS-math.abs(ld-last_deald[1])*comis*offS
	last_deald[1] = ld
	if math.abs(last_deald[3]) > 1.9*offS then
		eon = EXCH.."-pp"..tostring(price_part).."c"..tostring(price_partc).."-dp"..tostring(dpart).."c"..tostring(dpartc).."-"..ver..".txt"
		efilename = path_name.."error-GLASS-"..eon
		filer_err=io.open(efilename, "a")
		filer_err:write("=== position_down2 math.abs(last_deald[3]) > 1.9*offS ==="..eon, '\n')
		filer_err:write(SEC, '\n')
		filer_err:write("res="..res, '\n')
		filer_err:write("offS="..offS)
		filer_err:write(" ld_"..last_deald[1])
	...
	if (last_deald[1] ~= 0 and last_deald[5] == 0.) or (last_deald[1] == 0 and last_deald[5] ~= 0.) then
			eon = EXCH.."-pp"..tostring(price_part).."c"..tostring(price_partc).."-dp"..tostring(dpart).."c"..tostring(dpartc).."-"..ver..".txt"
			efilename = path_name.."error-GLASS-"..eon
			filer_err=io.open(efilename, "a")
...
function position_dn(SEC, last_deald)
	...
	elseif last_deald[1] == 0 then
			ld = -dub_up
			last_deald[5] = last_deald[7] * dub_up
	end
	last_deald[3]=last_deald[3]-(ld-last_deald[1])*bidS-math.abs(ld-last_deald[1])*comis*bidS
	last_deald[1] = ld
	if math.abs(last_deald[3]) > 1.9*bidS then
		eon = EXCH.."-pp"..tostring(price_part).."c"..tostring(price_partc).."-dp"..tostring(dpart).."c"..tostring(dpartc).."-"..ver..".txt"
		efilename = path_name.."error-GLASS-"..eon
		filer_err=io.open(efilename, "a")
		filer_err:write("=== position_down2 math.abs(last_deald[3]) > bidS ==="..eon, '\n')
		filer_err:write(SEC, '\n')
		filer_err:write("res="..res, '\n')
		filer_err:write("bidS="..bidS)
		filer_err:write(" ld_"..last_deald[1])
