...
dpart = 11 --коэффициент превышения суммы объема бид и оффер для сделки открытия позиции
-- например: сумма бидов умноженная на dpart > суммы офферов - покупка
dpartc = 11 -- то же для сделки закрытия позиции
ver = "v71-TEST" -- "WORK"
...
-- ver
				...
				for i=1,qsummax do 
					...
					elseif last_deal[i][SEC][1] > -dub_up then
