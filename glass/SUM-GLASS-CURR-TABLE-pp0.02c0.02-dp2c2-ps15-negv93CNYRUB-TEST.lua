...
price_part = 0.02 -- цена выставления ордера между средним (бид и оффер) и бид/оффер макс в интервале [0.0-1.0]
price_partc = 0.02 -- то же для сделки закрытия позиции
...
function OnQuote(class_code, sec_code)
	if class_code==CLASS and SEC==sec_code and (tm<stime or tm>=etime) and tm>=start_time and tm<end_time then
		...
		if tonumber(qt.bid_count)>=qsummax and tonumber(qt.offer_count)>=qsummax  then
			...
			ir = math.max(ir, summary_res[6], ps)
