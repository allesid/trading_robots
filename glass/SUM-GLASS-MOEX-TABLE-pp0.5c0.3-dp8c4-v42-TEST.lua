...
dpartc = 4 -- то же для сделки закрытия позиции

price_part = 0.5 -- цена выставления ордера между бид мин и бид макс в интервале [0.0-1.0]
price_partc = 0.3 -- то же для сделки закрытия позиции
ver = "v42-TEST" -- "WORK"
first_qs_num = 10
--  ========    DATA    ====================
...
-- last_deal[][SEC][6] - last order price [+-0]
	...
	for i=1, qsummax-first_qs_num+1 do 
		...
		if math.abs(last_deal[i][SEC][4]) > 100 then
			message("=== Error load math.abs(last_deal[SEC][4]) > 100 ===", 1)
			message(SEC, 1)
			message("res="..res, 1)
			message("last_deal[SEC][1]="..last_deal[SEC][1], 1)
			message("last_deal[SEC][2]="..last_deal[SEC][2], 1)
			message("last_deal[SEC][3]="..last_deal[SEC][3], 1)
			message("last_deal[SEC][4]="..last_deal[SEC][4], 1)
			message("last_deal[SEC][5]="..last_deal[SEC][5], 1)
	...
	while is_run do
		...
		if os.sysdate().min ~= tm_minute then
			...
			for i=1, qsummax-first_qs_num+1 do 
				dvl_sum = 0
				for row, SEC in pairs(sec_list) do
					pct=set_sell_neg(tb_res, row, SEC, last_deal, i)
					dvl_sum = dvl_sum + pct
				end
...
-- ===============================================
					...
					qt_of_sum = 0
					for j=1,i do
						qt_bid_sum = qt_bid_sum + tonumber(qt.bid[1+qt.bid_count-j].quantity)
						qt_of_sum = qt_of_sum + tonumber(qt.offer[j].quantity)
					end
					...
					if qt_bid_sum*dpart < qt_of_sum then
						if last_deal[i][SEC][6]>=0 and last_deal[i][SEC][1]>=(1-dub_up) then
					...
					elseif qt_bid_sum > qt_of_sum*dpart then
						if  last_deal[i][SEC][6]<=0 and last_deal[i][SEC][1]<=0 then
							PRICE_STEP=getSecurityInfo(CLASS, SEC).min_price_step
...
--  ============================================
function deal_stop(SEC, last_dealc, row, tb_res, ic)
	if last_dealc[SEC][1] ~= 0 then
		if last_dealc[SEC][1] > 0 then
			prc = prc_bid
		elseif last_dealc[SEC][1] < 0 then
			prc = prc_off
		end
		SetCell(tb_res, row, 2*ic+4, "stop")
		-- SetColor(tb_res, row, 2*ic+4, RGB(255,255,255),RGB(0,0,0), RGB(255,255,255), RGB(0,0,0))
		last_dealc = position_null(SEC, last_dealc)
		filer[ic]:write(SEC.." stop: date="..dt.." time="..tm.." res="..last_dealc[SEC][2].." price="..prc, '\n')
	end
	return last_dealc
end
--  ============================================
...
function position_up(SEC, last_dealu)
	...
	if last_deal[SEC][1] < 0  then
		...
		if math.abs(last_deal[SEC][4]) > 100 then
			message("=== Error position_up math.abs(last_deal[SEC][4]) > 100 ===", 1)
			message(SEC, 1)
			message("res="..res, 1)
			message("last_deal[SEC][1]="..last_deal[SEC][1], 1)
			message("last_deal[SEC][2]="..last_deal[SEC][2], 1)
	...
	last_deal[SEC][3]=last_deal[SEC][3]-(ld-last_deal[SEC][1])*offS-math.abs(ld-last_deal[SEC][1])*comis*offS
	last_deal[SEC][1] = ld
	return last_deal
end
--  ============================================
function position_dn(SEC, last_deald)
	...
	if last_deal[SEC][1] > 0 then
		...
		last_deal[SEC][4] = last_deal[SEC][4] + (res - last_deal[SEC][2])*100/math.abs(last_deal[SEC][5])
		if math.abs(last_deal[SEC][4]) > 100 then
			message("=== Error position_dn math.abs(last_deal[SEC][4]) > 100 ===", 1)
			message(SEC, 1)
			message("res="..res, 1)
			message("last_deal[SEC][1]="..last_deal[SEC][1], 1)
			message("last_deal[SEC][2]="..last_deal[SEC][2], 1)
...
--  ============================================
function set_sell_neg(tb_res, row, SEC, last_deal, ic)
	-- message(SEC.." "..vers, 1)
	SetCell(tb_res, row, 2*ic+4, string.format("%.0f", last_deal[ic][SEC][1]))
	-- SCALE=getSecurityInfo(CLASS, SEC).scale
	-- price_format = '%.'..tostring(SCALE)..'f'
	price_format = '%.2f'
	qt = getQuoteLevel2(CLASS, SEC)
	res = last_deal[ic][SEC][2]
	percent = last_deal[ic][SEC][4]
	if qt and qt.offer and tonumber(qt.offer_count) ~= 0 then
		if last_deal[ic][SEC][5] < 0 then
			offS = tonumber(qt.offer[1].price)
			res = last_deal[ic][SEC][3]+(last_deal[ic][SEC][1])*offS-math.abs(last_deal[ic][SEC][1])*comis*offS
		end
		if last_deal[ic][SEC][5] > 0 then
			bidS = tonumber(qt.bid[qt.bid_count-0].price)
			res = last_deal[ic][SEC][3]+(last_deal[ic][SEC][1])*bidS-math.abs(last_deal[ic][SEC][1])*comis*bidS
		end
	end
	if last_deal[ic][SEC][5] ~= 0 then
		percent = last_deal[ic][SEC][4] + (res - last_deal[ic][SEC][2])*100/math.abs(last_deal[ic][SEC][5])
	end
...
function prints(sec_list, last_dealc)
	...
	for i, SEC in pairs(sec_list) do
		...
		if math.abs(last_dealc[SEC][4]) > 100 then
			...
			message(SEC, 1)
			message("res="..res, 1)
			message("last_deal[SEC][1]="..last_dealc[SEC][1], 1)
			message("last_deal[SEC][2]="..last_dealc[SEC][2], 1)
			message("last_deal[SEC][3]="..last_dealc[SEC][3], 1)
			message("last_deal[SEC][4]="..last_dealc[SEC][4], 1)
