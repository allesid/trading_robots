...
static int forLua_GetTable(lua_State* ctx) {
	...
	//size_t visited = 0;
	int m_numcolumns = 2;
	int** m = create_array2d(bid_count + offer_count, m_numcolumns);
	int i0 = 0;
	...
	while (lua_next(ctx, t) != 0) {
		...
		m[i0][0] = lua_tonumber(ctx, -1) / price_step;
		lua_getfield(ctx, -2, "quantity");
		//quantity[i0] = lua_tointeger(ctx, -1);
		m[i0][1] = lua_tointeger(ctx, -1);
		++i0;
		lua_pop(ctx, 3);
	};
	lua_pop(ctx, 1);
	...
	//};
	int j = 0;
	for (int i = 0; i < bid_count + offer_count; ++i) {
		for (int j = 0; j < m_numcolumns; ++j) {
			lua_pushinteger(ctx, m[i][j]);
		};
	};
	free_array2d(m, bid_count + offer_count, m_numcolumns);
