...
vers = "v6.1dub-bs-TEST"
first_candle0 = -3170
end_candle0 = 0
ma_per1, ma_per2 = 2, 150
...
CreateWindow(tb_res)
	...
	for ma_per=ma_per1, ma_per2 do
		...
		for i, SEC in pairs(sec_list) do
			...
			res_max = 0
			curr_min = 0
			adx, pdi, mdi = psdf(first_candle, ds, SEC, ds[SEC]:C(1), ds[SEC]:C(1), ds[SEC]:C(1), ma_per)
			pdi_level = pdi
			mdi_level = mdi
			for j=first_candle,idx+end_candle0 do
				...
				pdi_level = (pdi_level*(ma_per-1) + pdi * 2)/(ma_per + 1)
				mdi_level = (mdi_level*(ma_per-1) + mdi * 2)/(ma_per + 1)
				ld_1 = ld
				if adx > adx_1 and pdi_1 < pdi_level then
					if pdi > pdi_1 then
						ld = 1
					end
				elseif adx < adx_1 and mdi_1 < mdi_level then
					if mdi > mdi_1 then
						ld = -dub_up
					end
				end
				...
				if ld ~= ld_1 then
					...
					if ma_per1 == ma_per2 then
						label(ld, prc, j, ds[SEC], res, chart_tag)
					end
				end
			...
			if ld ~= 0 then
				...
				res = pos + ld * prc
				if ma_per1 == ma_per2 then
					label(0, prc, idx, ds[SEC], res, chart_tag)
				end
			end
			respct = res*100/ds[SEC]:O(first_candle)
			if res_min > respct then
