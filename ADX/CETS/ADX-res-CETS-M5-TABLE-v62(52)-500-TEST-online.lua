...
end_of_name = vers.."-"..EXCH.."-M"..time_interval.."-l"..length_candles..".txt"
...
-- last_deal[SEC][6] - open position in lots
	...
	filer:write("vers="..vers, '\n')		
	filer:write("res_variant="..tostring(res_variant), '\n')		
...
CreateWindow(tb_res)
	...
	for q, SEC in pairs(sec_list) do
		...
		-- end
		-- last_deal = calc_deal(last_deal, CLASS, SEC, ds[SEC], idx, rv[SEC][6], time_interval, rv[SEC][5])
...
-- last_deal[SEC][6] - open position in lots
		...
		if tm_pr ~= tm then
			...
			-- psdf(j, ds, SEC, adx0, pdi0, mdi0, ma_per)
				...
				last_deal = calc_deal(last_deal, CLASS, SEC, ds[SEC], idx, ld, time_interval, rv[SEC][5])
				sum_res = sum_res + last_deal[SEC][4]
				if ld ~= ld_1 then
...
--  ============================================
function calc_deal(last_deal, CLASS, SEC, ds0, idx, ld, time_interval, ma_per)
		local ld_1 = last_deal[SEC][1]
		if ma_per >= ma_per_level then
			if dub_up == 1 then
				ld = -ld
			else
				if ld == 0 then
		...
		if ld ~= ld_1 then
			qt = getQuoteLevel2(CLASS, SEC)
			-- for i, v in pairs(qt.bid[1]) do
				-- message(SEC.." i= "..i.." v= "..tostring(v))
			-- end
			
			
			if tonumber(qt.bid_count) == 0 then
				qt.bid_count = 1
				qt.bid = {{price = ds0:C(idx)}} --{"1" = 8}
			end
			if tonumber(qt.offer_count) == 0 then
				qt.offer_count = 1
				qt.offer = {{price = ds0:C(idx)}}
				-- qt.offer[1].price = ds0:C(idx)
			end
			-- last_deal[SEC][6] =  -- open position in lots
