...
vers = "v3.3.3dub-indP-TEST-online"
first_candle0 = 0
end_candle0 = 0
...
-- message(SEC.." 1 time="..tostring(tm).." ldd="..tostring(ldd).." ldd_1="..tostring(ldd_1).." ld[1]="..tostring(last_deal[SEC][1]).." ld="..tostring(ld).." ld2="..tostring(ld2), 1)
	...
	-- last_deal[SEC][5] - last price
			...
			SetCell(tb_res, #sec_list+4+q, 8, tostring(ma_per_t[SEC]["ma_per_p"][1]).." "..tostring(ma_per_t[SEC]["ma_per_p"][2]))
		end
...
--  ============================================ os.sysdate().hour * 10000 + os.sysdate().min * 100
...
function best_ma_per(max_ma_per, sec_list, comis, tb_res, ds)
	...
	for i, SEC in pairs(sec_list) do
		...
		sumpct = 0
		ma_per_t[SEC] = {profit_d=0, ma_per_d=0, profit_p=0, ma_per_p={0, 0}}
		profit_max_mp = 0
-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		-- for ma_per=5, 5 do
		...
		-- label(ldd, prc, j, ds[SEC], respct, chart_tag)
				...
				if profit_max_mp < respct then
					...
					ma_per_t[SEC]["ma_per_p"] = {ma_per, ma_per2}
					ldd_p = ldd
				end
		...
		SetCell(tb_res, i, 8, tostring(ma_per_t[SEC]["ma_per_p"][1]).." "..tostring(ma_per_t[SEC]["ma_per_p"][2]))
