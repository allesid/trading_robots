...
vers = "v4.1dub-bs-TEST"
first_candle0 = -750
end_candle0 = 0
ma_per1, ma_per2 = 2, 100
...
CreateWindow(tb_res)
	...
	for ma_per=ma_per1, ma_per2 do
		...
		for i, SEC in pairs(sec_list) do
			...
			res_max = 0
			curr_min = 0
			adx_max = psdf(first_candle, ds, SEC, adx, pdi, mdi, ma_per)
			adx_min = adx_max
			adx_level_max= adx_max
			adx_level_min= adx_max
			for j=first_candle,idx+end_candle0 do
				...
				if adx > adx_max then
					adx_max = adx
					adx_level_max = (adx_max*2 + adx_min)/3
					message("ma_per="..ma_per.." adx_level_max="..string.format("%.1f", adx_level_max).." adx_max="..string.format("%.1f", adx_max).." adx_min="..string.format("%.1f", adx_min), 1)
				end
				if adx < adx_min then
					adx_min = adx
					adx_level_min = (adx_max + adx_min*2)/3
					message("ma_per="..ma_per.." adx_level_min="..string.format("%.1f", adx_level_min).." adx_max="..string.format("%.1f", adx_max).." adx_min="..string.format("%.1f", adx_min), 1)
				end
				ld_1 = ld
				if adx > adx_1 and adx < adx_level_min then
					if pdi > pdi_1 then
						ld = 1
					end
				elseif adx < adx_1 and adx > adx_level_max then
					if mdi > mdi_1 then
						ld = -dub_up
					end
				end
