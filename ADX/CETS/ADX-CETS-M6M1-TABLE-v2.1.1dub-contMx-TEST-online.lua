...
tih = INTERVAL_M6 -- time_interval_high
til = INTERVAL_M1 -- time_interval_low
comis = 0.0
max_ma_per = 50
--------------------------------------------
...
CreateWindow(tb_res)
	...
	for q, t in pairs(ti_t) do
		...
		for q, SEC in pairs(sec_list) do
			...
			for j=2,idx-1 do
				...
				-- mdi_1 = mdi
				-- adx_1 = adx
				adx, pdi, mdi = psdf(j, ds[t][SEC], adx, pdi, mdi, ma_per)
			end
	...
	for q, SEC in pairs(sec_list) do
		if last_deal[SEC][1] > 0 then
			ADXT[ti_t[1]][SEC][5] = 1
			ADXT[ti_t[2]][SEC][5] = -dub_up
		elseif last_deal[SEC][1] < 0 then
			ADXT[ti_t[1]][SEC][5] = -dub_up
			ADXT[ti_t[2]][SEC][5] = 1
		else
			ADXT[ti_t[1]][SEC][5] = 0
			ADXT[ti_t[2]][SEC][5] = 0
	...
	for ma_per=2, max_ma_per do
		...
		for i, SEC in pairs(sec_list) do
			...
			for j=first_candle_h[SEC],idx do
				...
				time_h_p = tonumber(ds[ti_t[1]][SEC]:T(j).hour * 100 + ds[ti_t[1]][SEC]:T(j).min + ti_t[1])
