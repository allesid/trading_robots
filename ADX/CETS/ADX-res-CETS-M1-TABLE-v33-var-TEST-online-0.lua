...
CreateWindow(tb_res)
	...
	for i, SEC in pairs(sec_list) do
		idx=ds[SEC]:Size()
		MA[SEC] = best_ma_per(SEC, idx, ds, max_ma_per)		-- SetCell(tb_res, row, 9, string.format('%.2f', pmin))
		...
		SetCell(tb_res, row, 4, string.format(price_format, MA[SEC][8][res_variant][1]))
		SetCell(tb_res, row, 5, string.format(price_format, MA[SEC][8][res_variant][2]))
		SetCell(tb_res, row, 6, string.format(price_format, MA[SEC][8][res_variant][3]))
		SetCell(tb_res, row, 7, string.format(price_format, MA[SEC][8][res_variant][4]))
		SetCell(tb_res, row, 8,  string.format('%.0f', MA[SEC][8][res_variant][5]))
		SetCell(tb_res, row, 9,  string.format('%.0f', MA[SEC][8][res_variant][6]))
	...
	while is_run do
		...
		for row, SEC in pairs(sec_list) do
			...
			if idx ~= MA[SEC][1]  then
				...
				ld = MA[SEC][8][res_variant][6]
				-- if MA[SEC][2] > MA[SEC][5] then
					-- if MA[SEC][3] > MA[SEC][6] then
						-- ld = 1
					-- elseif MA[SEC][4] > MA[SEC][7] then
						-- ld = -dub_up
					-- end
				-- end
