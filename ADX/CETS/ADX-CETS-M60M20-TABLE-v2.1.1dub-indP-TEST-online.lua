...
vers = "v2.1.1dub-indP-TEST-online"
first_candle0 = 0
end_candle0 = 0
-- ma_per1, ma_per2 = 2, 350
tih = INTERVAL_H1 -- time_interval_high
til = INTERVAL_M20 -- time_interval_low
comis = 0.0005
max_ma_per = 50
--------------------------------------------
	...
	for q, t in pairs(ti_t) do
		...
		for q, SEC in pairs(sec_list) do
			...
			for j=2,idx-1 do
				...
				mdi_1 = mdi
				adx_1 = adx
				adx, pdi, mdi = psdf(j, ds[t][SEC], adx_1, pdi_1, mdi_1, ma_per[SEC]["ma_per_p"]})
			end
		...
		for q, SEC in pairs(sec_list) do
			...
			for q0, t in pairs(ti_t) do
				...
				if tonumber(idx) > tonumber(ADXT[t][SEC][4]) then
					...
					adx = 50
					for j=2,idx-1 do
						adx, pdi, mdi = psdf(j, ds[t][SEC], adx, pdi, mdi, ma_per[SEC]["ma_per_p"])
					end
					ADXT[t][SEC][1] = adx
					ADXT[t][SEC][2] = pdi
					ADXT[t][SEC][3] = mdi
			...
			adx, pdi, mdi = psdf(ds[ti_t[1]][SEC]:Size(), ds[ti_t[1]][SEC], ADXT[ti_t[1]][SEC][1], ADXT[ti_t[1]][SEC][2], ADXT[ti_t[1]][SEC][3], ma_per)
			ld_1 = ld
			...
			last_deal[SEC][6] = ld
			adx2, pdi2, mdi2 = psdf(ds[ti_t[2]][SEC]:Size(), ds[ti_t[2]][SEC], ADXT[ti_t[2]][SEC][1], ADXT[ti_t[2]][SEC][2], ADXT[ti_t[2]][SEC][3], ma_per)
...
-- last_deal[SEC][5] - last price
		-- respct = res*100/ds[SEC]:O(idx+first_candle0)
			SetCell(tb_res, q, 4, string.format('%.0f', ld).." "..string.format('%.0f', ld2))
			...
			SetCell(tb_res, q, 8, string.format('%.0f', ma_per[SEC]["ma_per_p"]))
...
-- end 
		...
		SetCell(tb_res, i, 8, string.format('%.0f', ma_per_t[SEC]["ma_per_d"]).."  "..string.format('%.0f', ma_per_t[SEC]["ma_per_p"]))

		-- message(SEC.." "..ma_per.." "..sumpct.." "..sumpct_glob.." "..sumpct_glob2.." "..ma_per_glob, 1)
