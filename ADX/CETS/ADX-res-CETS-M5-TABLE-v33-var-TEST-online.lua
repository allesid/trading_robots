	...
	while is_run do
		...
		for row, SEC in pairs(sec_list) do
			...
			if idx ~= MA[SEC][1]  then
				...
				prints(sec_list, last_deal)
				sum_res = 0
				for q, s in pairs(sec_list) do
					sum_res = sum_res + last_deal[s][4]
				end
				SetCell(tb_res, last_row, 4, string.format(price_format, sum_res))
			end
...
function calc_deal(last_deal, CLASS, SEC, ds0, idx, ld, time_interval)
		...
		if ld ~= ld_1 then
			...
			if ld == 1 then
				...
				res = last_deal[SEC][3] + ld_1 * prc
				res_pct = res*100/ds0:C(idx+last_candle)
				Blabel["YVALUE"]=prc
				Blabel["DATE"]=c_date
				Blabel["TIME"]=ds0:T(idx).hour*10000+ds0:T(idx).min*100
			...
			elseif ld == -1 then
				...
				Slabel["YVALUE"]=prc
				Slabel["DATE"]=c_date
				Slabel["TIME"]=ds0:T(idx).hour*10000+ds0:T(idx).min*100
				...
				STOPlabel["TIME"]=ds0:T(idx).hour*10000+ds0:T(idx).min*100
