...
vers = SEC.."-v6.2.3dub-indP-TEST-online"
first_candle0 = 0
end_candle0 = 0
...
-- table length == #sec_list + 5
	...
	for q, SEC in pairs(sec_list) do
		...
		SetCell(tb_res, row, 2, CLASS)
		SetCell(tb_res, row, 3, getSecurityInfo(CLASS, SEC).short_name)
		SetCell(tb_res, #sec_list+5+q, 4, string.format('%.0f', last_deal[SEC][6]).." "..string.format('%.0f', last_deal[SEC][7]))
...
-- table length == #sec_list * 2 + 6
	...
	for q, t in pairs(ti_t) do
		...
		for q, SEC in pairs(sec_list) do
			...
			ADXT[t][SEC] = {adx, pdi, mdi, adx_1, pdi_1, mdi_1, idx}
			-- a = ADXT[t][SEC]
	-- message("1 "..tostring(a[1]).." "..tostring(a[2]).." "..tostring(a[3]).." "..tostring(a[4]).." "..tostring(a[5]).." "..tostring(a[6]).." "..tostring(a[7]), 2)
...
-- message(SEC.." 1 time="..tostring(tm).." ldd="..tostring(ldd).." ldd_1="..tostring(ldd_1).." ld[1]="..tostring(last_deal[SEC][1]).." ld="..tostring(ld).." ld2="..tostring(ld2), 1)
		for q, SEC in pairs(sec_list) do
			for q0, t in pairs(ti_t) do
				...
				if tonumber(idx) > tonumber(ADXT[t][SEC][7]) then
				...
				-- message(SEC.." adx="..string.format('%.2f', adx).." pdi="..string.format('%.2f', pdi).." mdi="..string.format('%.2f', mdi).." tm="..string.format('%.0f', ds[t][SEC]:T(idx-1).hour*100+ds[t][SEC]:T(idx-1).min).." ma_per="..ma_per, 1)
					if t == ti_t[1] then
						...
						ld = last_deal[SEC][6]
						ld_1 = ld
						ld = calc_ld(ADXT[t][SEC], ld)
						last_deal[SEC][6] = ld
					else
						ld =last_deal[SEC][6]
						ld2 =last_deal[SEC][7]
						ld2_1 = ld2
						ld2 = calc_ld(ADXT[t][SEC], ld2)
						last_deal[SEC][7] = ld2
			...
			-- last_deal[SEC][5] - last price
				...
				end -- if tonumber(idx) > tonumber(ADXT[t][SEC][4]) then
			end -- for q0, t in pairs(ti_t) do
			SetCell(tb_res, #sec_list+5+q, 4, string.format('%.0f', last_deal[SEC][6]).." "..string.format('%.0f', last_deal[SEC][7]))
			SetCell(tb_res, #sec_list+5+q, 5, string.format('%.0f', last_deal[SEC][1]))
			SetCell(tb_res, #sec_list+5+q, 6, string.format(price_format, last_deal[SEC][2]))
			SetCell(tb_res, #sec_list+5+q, 7, string.format(price_format,last_deal[SEC][4]))
...
--  ============================================
function calc_ld(a, ld0)
	-- message(tostring(a[1]).." "..tostring(a[2]).." "..tostring(a[3]).." "..tostring(a[4]).." "..tostring(a[5]).." "..tostring(a[6]).." "..tostring(a[7]), 2)
	if a[1] > a[4] then
		if a[2] > a[5] then
			ld0 = 1
		end
...
function best_ma_per(max_ma_per, sec_list, comis, tb_res, ti_t, ds)
	...
	for i, SEC in pairs(sec_list) do
		...
		res_min_s = 0
		res_max_s = 0
		ma_per_t[SEC] = {profit_d=0, ma_per_d=0, profit_p=0, ma_per_p=0, ma_per_r=0, drawdown_p=-100, drawdown_d=-100}
		drawdown_min_mp = -100
		profit_max_mp = -100
-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		-- for ma_per=5, 5 do
		-- for ma_per=41, 41 do
		for ma_per=max_ma_per0, max_ma_per do
			...
			for j=first_candle_h[SEC],idx do
				...
				ld_1 = ld
				a = {adx, pdi, mdi, adx_1, pdi_1, mdi_1}
				ld = calc_ld(a, ld)
		...
		-- sleep(50)
				...
				a = {adx2, pdi2, mdi2, adx2_1, pdi2_1, mdi2_1}
				ld2 = calc_ld(a, ld2)
...
if max_ma_per0 == max_ma_per then
...
if max_ma_per0 == max_ma_per then
	label(0, prc, ds[ti_t[2]][SEC]:Size(), ds[ti_t[2]][SEC], respct, chart_tag)
end
			end
			if res_min > respct then
				res_min = respct
			...
			if profit_max_mp < respct then
				...
				ldd_p = ldd
				-- message(SEC.."  "..ma_per_t[SEC]["ma_per_p"].."  "..respct, 1)
			end
			...
			SetCell(tb_res, i, 5, string.format('%.2f', respct))
			SetCell(tb_res, i, 6, string.format('%.2f', ma_per_t[SEC]["profit_p"]))
