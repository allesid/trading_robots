	...
	for ma_per=ma_per1, ma_per2 do
		...
		for i, SEC in pairs(sec_list) do
			...
			for j=first_candle,idx+end_candle0 do
				...
				if adx > adx_max then
					adx_max = adx
					adx_level = (adx_max + adx_min)/2
					message("ma_per="..ma_per.." adx_level="..string.format("%.1f", adx_level).." adx_max="..string.format("%.1f", adx_max).." adx_min="..string.format("%.1f", adx_min), 1)
				end
				if adx < adx_min then
					adx_min = adx
					adx_level = (adx_max + adx_min)/2
