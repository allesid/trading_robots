...
-- table length == #sec_list * 2 + 4
	...
	for q, SEC in pairs(sec_list) do
		...
		last_deal[SEC] = calc_last_deal(ma_per[SEC]["ldd"], last_deal[SEC][1], ds[SEC], last_deal[SEC], filer, comis)
		idx=ds[SEC]:Size()
			pdi = 10
			mdi = 10
			adx = 10
		...
		ADXT[SEC] = {adx, pdi, mdi, idx, adx_1, pdi_1, mdi_1}
...
-- message(SEC.." 1 time="..tostring(tm).." ldd="..tostring(ldd).." ldd_1="..tostring(ldd_1).." ld[1]="..tostring(last_deal[SEC][1]).." ld="..tostring(ld).." ld2="..tostring(ld2), 1)
		...
		for q, SEC in pairs(sec_list) do
			...
			if ma_per[SEC]["ma_per"] ~= 0 then
				...
				if (tonumber(os.sysdate().hour*60)+tonumber(string.match(getInfoParam("SERVERTIME"), ":(%d%d):")))%ti > ti*PARTTIME then
					...
					last_deal[SEC] = calc_last_deal(ldd, ldd_1, ds[SEC], last_deal[SEC], filer, comis)
					if ldd_1 ~= last_deal[SEC][1] then
						label(ldd, last_deal[SEC][5], idx, ds[SEC], last_deal[SEC][2], chart_tag)
...
--  ============================================
function calc_last_deal(ldd, ldd_1, ds0, last_deal0, filer, comis)
	if ldd ~= ldd_1 then
		prc_1 = last_deal0[5]
		prc = ds0:C(ds0:Size())
		res_1 = last_deal0[2]
...
function calc_ld(adx, pdi, mdi, a, ld)
	if (adx > a[1] and a[1] < a[5]) or (adx < a[1] and a[1] > a[5]) then
		if pdi > a[2] then
			ld = 1
		end
		if mdi > a[3] then
...
function best_ma_per(i, max_ma_per, SEC, comis, tb_res, ds)
	...
	for ma_per=2, max_ma_per do
		...
		for j=2,idx do
			...
			ldd_1 = ldd
			ldd = calc_ld(adx, pdi, mdi, {adx_1, pdi_1, mdi_1, j, adx_2, pdi_2, mdi_2}, ldd)
