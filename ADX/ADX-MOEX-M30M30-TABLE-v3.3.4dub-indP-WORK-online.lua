...
vers = "v3.3.4dub-indP-WORK-online"
tih = INTERVAL_M30 -- time_interval_high
til = INTERVAL_M30 -- time_interval_low
comis = 0.0005
max_ma_per = 50
PARTTIME = 0
...
-- resd = sendTransaction(trans_S)
		-- message(SEC.." resd:"..tostring(resd).." SELL price="..tostring(prc).." tn= "..tostring(tn).." qty="..trans_S["QUANTITY"], 1)
		-- filer:write(SEC.." SELL resd:"..resd.." SELL price="..tostring(prc).." tn= "..tostring(tn).." Trvol= "..tostring(Trvol).." qty="..trans_S["QUANTITY"], '\n')
...
-- resd = sendTransaction(trans_B)
		...
		-- filer:write(SEC.." BUY  resd:"..resd.." SELL price="..tostring(prc).." tn= "..tostring(tn).." Trvol= "..tostring(Trvol).." qty="..trans_S["QUANTITY"], '\n')
...
-- table length == #sec_list * 2 + 6
	...
	sumpct = 0
	message("Server time : "..getInfoParam("SERVERTIME").." mints=".. string.match(getInfoParam("SERVERTIME"), ":(%d%d):"), 1)
	message("Clock time : "..tostring(os.sysdate().hour * 100 + os.sysdate().min * 1), 1)
	for q, SEC in pairs(sec_list) do
		ma_per[SEC] = best_ma_per(q, max_ma_per, SEC, comis, tb_res, ds)
		sumpct = sumpct + ma_per[SEC]["profit_p"]
		if last_deal[SEC][1] ~= ma_per[SEC]["ldd"] then
			message("ATTENTION! ld change! "..SEC.." : "..last_deal[SEC][1].." ~= "..ma_per[SEC]["ldd"], 2)
			last_deal[SEC] = calc_ldd(0, 0, ma_per[SEC]["ldd"], last_deal[SEC], idx, ds[SEC], chart_tag)
		end
...
-- message(SEC.." 1 time="..tostring(tm).." ldd="..tostring(ldd).." ldd_1="..tostring(ldd_1).." ld[1]="..tostring(last_deal[SEC][1]).." ld="..tostring(ld).." ld2="..tostring(ld2), 1)
		...
		for q, SEC in pairs(sec_list) do
			if ma_per[SEC]["ma_per_p"][1] ~= 0 and ma_per[SEC]["ma_per_p"][2] ~= 0 then
				...
				if (tonumber(os.sysdate().hour*60)+tonumber(string.match(getInfoParam("SERVERTIME"), ":(%d%d):")))%tih > tih*PARTTIME then
	...
	-- message(SEC.." adx2_1="..string.format('%.4f', ADXT[2][SEC][1]).." pdi2_1="..string.format('%.4f', ADXT[2][SEC][2]).." mdi2_1="..string.format('%.4f',ADXT[2][SEC][3]) , 1)
					...
					last_deal[SEC] = calc_ldd(ld, ld2, last_deal[SEC][1], last_deal[SEC], idx, ds[SEC], chart_tag)
					ldd = last_deal[SEC][1]
					if ldd ~= ldd_1 then
						prints(sec_list, last_deal)
					end
					...
					if TW == "WORK" then
						if getSecurityInfo(CLASS, SEC) == nil  then
							...
							if getDepoEx(FIRM_ID, CLIENT_CODE, SEC, TRADE_ACC, 365) ~= nil then
								...
								if tn ~= Trvol then
									message("Server time : "..getInfoParam("SERVERTIME"))
									...
									message(SEC.." vers="..vers.." tih="..tostring(tih).." minutes="..string.match(getInfoParam("SERVERTIME"), ":(%d%d):"), 2)
									message(SEC.." LOTS_VALUE="..tostring(LOTS_VALUE).." tn="..tostring(tn).." Trvol="..tostring(Trvol), 2)
									if Trvol > tn then
										buy_now(tn, Trvol, SEC, price_format, PRICE_STEP, trans_B)
										filer:write(SEC.." BYE  time="..tostring(tm).." tn="..tostring(tn).." Trvol="..tostring(Trvol).." ld[sec]="..tostring(last_deal[SEC][1]).." lots="..tostring(ldd-ldd_1).." ldd="..tostring(ldd).." prc="..string.format('%.2f', prc).." res="..tostring(last_deal[SEC][2]).." pct="..tostring(last_deal[SEC][4]).." ld="..tostring(ld).." ld2="..tostring(ld2), '\n')
									elseif Trvol < tn then
										sell_now(tn, Trvol, SEC, price_format, PRICE_STEP, trans_S)
										filer:write(SEC.." SELL time="..tostring(tm).." tn="..tostring(tn).." Trvol="..tostring(Trvol).." ld[sec]="..tostring(last_deal[SEC][1]).." lots="..tostring(ldd-ldd_1).." ldd="..tostring(ldd).." prc="..string.format('%.2f', prc).." res="..tostring(last_deal[SEC][2]).." pct="..tostring(last_deal[SEC][4]).." ld="..tostring(ld).." ld2="..tostring(ld2), '\n')
									end
								end
							end
						end
...
--  ============================================
function calc_ldd(ld, ld2, ldd, last_deal0, idx, ds0, chart_tag)
	ldd_1 = last_deal0[1]
	if ld == 1 then
		if ld2 <= 0 then
			ldd = 1
	...
	if ldd ~= ldd_1 then
		...
		prc = ds0:C(ds0:Size())
		res_1 = last_deal0[2]
		last_deal0[2] = last_deal0[3] + ldd_1 * prc
		last_deal0[3] = last_deal0[3] - (ldd-ldd_1) * prc - math.abs(ldd-ldd_1) * comis * prc
		last_deal0[5] = prc
		last_deal0[1] = ldd
		if ldd_1 ~= 0 then
			last_deal0[4] = last_deal0[4] + (last_deal0[2] - res_1)*100/prc_1
		end
		label(ldd, prc, idx, ds0, last_deal0[2], chart_tag)
		tm = os.sysdate().hour * 100 + os.sysdate().min

		if ldd > ldd_1 then
			filer:write(SEC.." BYE  time="..tostring(tm).." lots="..tostring(ldd-ldd_1).." ldd="..tostring(ldd).." prc="..string.format('%.2f', prc).." res="..tostring(last_deal0[2]).." pct="..tostring(last_deal0[4]).." ld="..tostring(ld).." ld2="..tostring(ld2), '\n')
			message(SEC.." "..vers.." BYE  time="..tostring(tm).." lots="..tostring(ldd-ldd_1).." ldd="..tostring(ldd).." prc="..string.format('%.2f', prc).." res="..tostring(last_deal0[2]).." pct="..tostring(last_deal0[4]).." ld="..tostring(ld).." ld2="..tostring(ld2), 1)
		else
			filer:write(SEC.." SELL time="..tostring(tm).." lots="..tostring(ldd-ldd_1).." ldd="..tostring(ldd).." prc="..tostring(prc).." res="..tostring(last_deal0[2]).." pct="..tostring(last_deal0[4]).." ld="..tostring(ld).." ld2="..tostring(ld2), '\n')
			message(SEC.." "..vers.." SELL time="..tostring(tm).." lots="..tostring(ldd-ldd_1).." ldd="..tostring(ldd).." prc="..string.format('%.2f', prc).." res="..tostring(last_deal0[2]).." pct="..tostring(last_deal0[4]).." ld="..tostring(ld).." ld2="..tostring(ld2), 1)
		end
...
function best_ma_per(i, max_ma_per, SEC, comis, tb_res, ds)
	...
	-- end
		...
		time_h = ds[SEC]:T(1).hour * 10000 + ds[SEC]:T(1).min * 100 + ds[SEC]:T(1).sec
		time_l = ds[SEC]:T(1).hour * 10000 + ds[SEC]:T(1).min * 100 + ds[SEC]:T(1).sec
	...
	ma_per_t = {profit_p=0, ma_per_p={0, 0}, ldd=0}
	profit_max_mp = 0
-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	-- for ma_per=5, 5 do
