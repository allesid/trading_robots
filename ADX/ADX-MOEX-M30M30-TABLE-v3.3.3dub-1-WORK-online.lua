...
-- message(SEC.." 1 time="..tostring(tm).." ldd="..tostring(ldd).." ldd_1="..tostring(ldd_1).." ld[1]="..tostring(last_deal[SEC][1]).." ld="..tostring(ld).." ld2="..tostring(ld2), 1)
	...
	-- message(SEC.." adx2_1="..string.format('%.4f', ADXT[2][SEC][1]).." pdi2_1="..string.format('%.4f', ADXT[2][SEC][2]).." mdi2_1="..string.format('%.4f',ADXT[2][SEC][3]) , 1)
					...
					if adx > ADXT[1][SEC][1] then
						if pdi > ADXT[1][SEC][2] and mdi < ADXT[1][SEC][3] then
							ld = 1
						end
						if mdi > ADXT[1][SEC][3] and pdi < ADXT[1][SEC][2] then
							ld = -dub_up
						end
					...
					if adx2 > ADXT[2][SEC][1] then
						if pdi2 > ADXT[2][SEC][2] and mdi2 < ADXT[2][SEC][3] then
							ld = 1
						end
						if mdi2 > ADXT[2][SEC][3] and pdi < ADXT[2][SEC][2] then
							ld = -dub_up
						end
...
--  ============================================
function calc_ldd(ldd, ld, ld2)
	if ld == 1 then
		if ld2 <= 0 then
			ldd = 1
		end
		-- if ldd == -1 and ld2 == 1 then
			-- ldd = 0
		-- end
	elseif ld == 0 then
		if dub_up == 0 then
